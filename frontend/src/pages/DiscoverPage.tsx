...
export default function DiscoverPage({ onLogout: _onLogout }: { onLogout: () => void }) {
    ...
    const [includeMyTaste, setIncludeMyTaste] = useState(false);
        ...
        try {
            const data = await api<DiscoverResult>("/discover", {
                ...
                body: {
                    ...
                    save_to_playlist: saveToPlaylist,
                    include_my_taste: includeMyTaste,
                },
                token: token || "",
            });
            setResult(data);
            ...
            <div className="mx-auto max-w-2xl">
                ...
                <form onSubmit={handleDiscover} className="mb-8">
                    <div className="glass rounded-2xl p-4">
                        ...
                        <div className="mt-3 flex flex-wrap gap-3 border-t border-white/5 pt-3">
                            <label className="flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-1.5 transition hover:bg-white/5">
                                <div className="relative">
                                    <input
                                        type="checkbox"
                                    ...
                                    />
                                    <div className="h-5 w-9 rounded-full bg-gray-700 transition peer-checked:bg-green-500" />
                                    <div className="absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white transition peer-checked:translate-x-4" />
                                </div>
                                <span className="text-xs text-gray-400 peer-checked:text-gray-200">Save to Playlist</span>
                            </label>
                            <label className="flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-1.5 transition hover:bg-white/5">
                                <div className="relative">
                                    <input
                                        type="checkbox"
                                        checked={includeMyTaste}
                                        onChange={(e) => setIncludeMyTaste(e.target.checked)}
                                        className="peer sr-only"
                                    />
                                    <div className="h-5 w-9 rounded-full bg-gray-700 transition peer-checked:bg-purple-500" />
                                    <div className="absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white transition peer-checked:translate-x-4" />
                                </div>
                                <span className="text-xs text-gray-400 peer-checked:text-gray-200">Include my taste</span>
                            </label>
                        </div>
                ...
                {result && (
                    <div>
                        ...
                        {/* Playlist created banner */}
                        {result.playlist_url && (
                            <div className="mb-4 rounded-xl bg-purple-500/10 p-4 ring-1 ring-purple-500/20">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <span className="text-2xl">📋</span>
                                        <div>
                                            <p className="text-sm font-semibold text-purple-300">
                                                Playlist created: {result.playlist_name}
                                            </p>
                                            <p className="text-xs text-gray-400">
                                                {result.songs.filter((s) => s.spotify_uri).length} songs added to your Spotify
                                            </p>
                                        </div>
                                    </div>
                                    <a
                                        ...
                                        className="flex items-center gap-1.5 rounded-lg bg-green-500 px-3 py-1.5 text-xs font-semibold text-gray-950 transition hover:bg-green-400"
                                    >
